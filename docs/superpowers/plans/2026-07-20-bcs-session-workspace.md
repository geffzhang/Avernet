...
// encode / decode_and_verify / decode_and_verify_no_expiry 将在 Step 3 实现。
...
- [ ] **Step 4: 实现 encode / decode_and_verify**
...
impl SessionFileRepoPort for MemorySessionFileRepo {
    ...
    async fn list_expired_pending(&self, now: u64, limit: u32) -> ServiceResult<Vec<SessionFile>> {
        ...
        let mut out: Vec<SessionFile> = rows.values()
            .filter(|r| r.status == FileStatus::Pending && r.object_handle.contains('"')) // row carries expires_at in handle
            .filter(|r| {
                // expires_at stored in object_handle envelope; for memory we also keep a side check via updated_at fallback
                r.updated_at < now // simplification: memory uses updated_at as proxy; mysql uses real expires_at column
            })
            .cloned().collect();
...
> **memory `list_expired_pending` 说明**：为与 mysql 对齐，`SessionFile` 域类型不单独含 `expires_at`（它隐于 `object_handle` envelope）。memory 实现只做语义近似（用于单测的 Pending sweep 行为验证，不验证真实过期）；mysql 实现从 `object_handle` JSON 解析 `expires_at` 或额外 SELECT 投影。**实现时统一**：在 `NewSessionFileParams` 已带 `expires_at`，repo 写入 `object_handle`（envelope 含 `expires_at`）；sweep 时 mysql 用 `JSON_EXTRACT(object_handle, '$.expires_at') < ?` 过滤，memory 写入时把 `expires_at` 也存入 envelope 由 sweep 解析。**修正**：让 memory `list_expired_pending` 也解析 envelope：
...
impl SessionFileService for SessionFileServiceImpl {
    ...
    async fn prepare_upload(&self, cmd: PrepareUploadCommand) -> Result<PrepareUploadResult, SessionFileUseCaseError> {
        if cmd.size > self.max_size() {
            return Err(SessionFileUseCaseError::InvalidInput(format!("size {} exceeds max {}", cmd.size, self.max_size())));
        }
...
fn err_to_response(err: SessionFileUseCaseError) -> Response {
    ...
    let (code, status) = match &err {
        ...
        InvalidInput(m) if m.contains("exceeds max") => ("PAYLOAD_TOO_LARGE", StatusCode::PAYLOAD_TOO_LARGE),

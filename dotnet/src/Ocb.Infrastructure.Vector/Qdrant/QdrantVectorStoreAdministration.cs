...
/// <summary>
/// Qdrant vector store administration — cluster deployment only.
/// </summary>
public sealed class QdrantVectorStoreAdministration : IVectorStoreAdministration
{
    private readonly string _profile;
    private readonly ConcurrentDictionary<string, CollectionMeta> _collections;

    public QdrantVectorStoreAdministration(
        string profile,
        ConcurrentDictionary<string, CollectionMeta>? collections = null)
    {
        if (!string.Equals(profile, "cluster", StringComparison.OrdinalIgnoreCase))
            throw new InvalidOperationException("Qdrant is cluster-only. Use SonnetDB for singlebox deployments.");
